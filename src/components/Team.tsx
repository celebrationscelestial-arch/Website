...
const App = () => {
  ...
  return (
    ...
    >
      ...
      {/* UPDATED: Reduced padding and text size for mobile */}
      <div className="min-h-[75vh] flex flex-col justify-center items-center text-center p-4 relative">
        ...
        <div className="relative z-10 bg-white/60 backdrop-blur-md p-6 sm:p-8 lg:p-12 rounded-2xl shadow-xl max-w-4xl mx-auto">
            {/* UPDATED: Reduced text size for mobile */}
            <h2 className="text-4xl sm:text-5xl lg:text-6xl font-serif font-bold text-gray-800">
                Why Choose Us
            </h2>
            <div className="w-24 h-1 bg-amber-400 mx-auto rounded-full my-6"></div>
            {/* UPDATED: Reduced text size for mobile */}
            <p className="text-lg sm:text-xl font-sans text-gray-600 max-w-3xl mx-auto leading-relaxed">
      ...
      <div className="relative">
        {features.map((feature, index) => (
          ...
          <div key={index} className="sticky top-0 h-screen w-full">
            {/* UPDATED: Reduced padding for mobile */}
            <div className={`h-full w-full flex items-center justify-center p-6 sm:p-8 lg:p-16 ${cardBackgrounds[index % cardBackgrounds.length]}`}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 max-w-6xl mx-auto items-center">
                ...
                <div className={`w-full h-[40vh] sm:h-[50vh] lg:h-[60vh] rounded-2xl overflow-hidden shadow-2xl ${index % 2 === 0 ? 'lg:order-1' : 'lg:order-2'}`}>
                ...
                <div className={`text-gray-800 ${index % 2 === 0 ? 'lg:order-2' : 'lg:order-1'}`}>
                  {/* UPDATED: Reduced text size for mobile */}
                  <h3 className="text-3xl sm:text-4xl lg:text-5xl font-serif font-bold mb-6 text-gray-800">{feature.title}</h3>
                  {/* UPDATED: Reduced text size for mobile */}
                  <p className="text-base sm:text-lg lg:text-xl font-sans text-gray-600 leading-relaxed">{feature.description}</p>
                </div>
